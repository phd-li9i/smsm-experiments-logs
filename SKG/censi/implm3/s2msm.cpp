  ...
  std::tuple<double,double,double>* virtual_pose)
{
  ...
  if (do_artificial_360)
  {
    ...
    *map = X::find(origin_of_generated_pose, *map, SIZE_MAP);
  }
  else // just impress the rays onto the plane over 2π
  {
    ...
    // This is the map
    *map = X::find(dataset_pose, *map, SIZE_MAP);
  }
  ...
  std::vector< std::pair<double,double> > intersections =
    X::find(*real_pose, *map, SIZE_REAL_SCAN);
  ...
  for (int n = 0; n < NUM_ITERATIONS; n++)
  {
    ...
    for (int i = START_SAMPLE; i < END_SAMPLE; i++)
    {
      ...
      generateMapConfiguration(dataset_real_pose, dataset_real_scan,
        ...
        &true_map, &real_scan, &real_pose, &virtual_pose);
      ...
      // -----------------------------------------------------------------------
      // This is the meat ------------------------------------------------------
      testOne(real_scan, real_pose, virtual_pose, true_map,
  ...
  std::tuple<double,double,double>* result_pose)
{
  ...
  if (METHOD.compare("FMT") == 0 || METHOD.compare("DBH") == 0
                                 || METHOD.compare("KU") == 0)
    Match::fmtdbh(real_scan_corrupted, virtual_pose, map_corrupted,
      METHOD, r2rp_, c2rp_, ip, op, result_pose);


  if (METHOD.compare("CSM") == 0)
    Match::csm(real_scan_corrupted, virtual_pose, map_corrupted,
      &input_, &output_, ip, op, result_pose);


  if (METHOD.compare("NDT") == 0)
    Match::ndt(real_scan_corrupted, virtual_pose, map_corrupted,
      ip, op, result_pose);
